...
\label{sec:clique}
We introduce two encodings. Section \ref{sec:vertex-edge} defines a graph where operation cost information is encoded as weights on both vertices and edges, while in Section \ref{sec:vertex} only vertices are weighted. Similar ideas with compatibility graphs have been widely explored (and successful) for several types of maximum common subgraph problems \cite{DBLP:conf/mco/VismaraV08}.
\subsubsection{Putting weights on both vertices and edges}
\label{sec:vertex-edge}
\begin{figure}
...
%  \end{equation}
%  Then the set of edit operations $\lambda$ corresponding to vertices in $C$ and all edges between them is a complete edit path from $G_1$ to $G_2$ with minimum total cost.
%\end{theorem}
%\begin{proof}
%  Suppose it is not a complete edit path. Then there is a vertex or edge in $G_1$ or $G_2$ which is not part of any operation in $\lambda$ (since one operation is always enough and there can be no "useless" or invalid operations). Having such a vertex would contradict condition \ref{eq:cond1} or condition \ref{eq:cond2}. Therefore it must be an edge. Pick an arbitrary edge $e(u_i, u_j) \in E_1$. Consider the edge $\phi \in E_3$ between vertices corresponding to operations $u_i \to v_i$, $u_j \to v_j$ for some $v_i, v_j \in V_2 \cup \{ \epsilon \}$ (they exist and are unique). If $e(v_i, v_j) \not \in E_2$ (or is not a valid edge), then $\zeta(\phi) = c(e(u_i, u_j), e(v_i, v_j)) = c(e(u_i, u_j), \epsilon)$, which means that $\phi$ represents a deletion. Otherwise $\zeta(\phi) = \min(c(e(u_i, u_j), e(v_i, v_j)), c(e(u_i, u_j), \epsilon) + c(\epsilon, e(v_i, v_j)))$, which makes it either a substitution or a deletion and an insertion. Either way $\phi \in E_3$ represents an operation on $e(u_i, u_j)$. Case for $e(u_i, u_j) \in E_2$ can be handled similarly.
%
%  Now suppose there is a complete edit path $\lambda'$ with a smaller total cost. Consider a subset of vertices $C' \subseteq V_3$ corresponding to $\lambda'$. 
%\end{proof}
