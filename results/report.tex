...
\end{proof}
% One could also prove a lower bound for size.
\begin{corollary}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Vertex-edge-weights encoding of the example from Figure \ref{fig:example} with the clique corresponding to the edit path \eqref{eqn:1} in blue}
  \label{fig:encoding1}
\end{figure}
...
\end{theorem}
% a minimal edit path is complete if and only if inequalities become equalities
\begin{proof}
...
\begin{algorithm}
  \Function{\colouringBound{$w, (S_i)_{i=0}^{k-1}, P, C, \mathit{cumulativeWtBound}$}}{
    ...
    \For{$i \gets 0, \dots, k-1$}{
      ...
      \If{$\mathit{independentSetSatisfied}[i] = 0$}{
        \If{$\mathit{howManyVerticesInP} == 0$}{
          \Return \textit{IMPOSSIBLE\_TO\_SATISFY}; \tcp{A negative integer constant}
        }
    ...
    \If{$\mathit{minVerticesInP} = \infty$}{
      \Return \textit{ALL\_CONSTRAINTS\_SATISFIED}; \tcp{A different negative integer constant}
    }
    \For{$i \gets 0, \dots, k-1$}{
