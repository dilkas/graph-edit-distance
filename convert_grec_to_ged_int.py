...
with open(common.new_filename(sys.argv[1:]), 'w') as f:
    ...
    for i, t1 in enumerate(vertex_type[0]):
        ...
        for j, t2 in enumerate(vertex_type[1]):
            substitutions.append(int(0.5 * math.sqrt((x[0][i] - x[1][j])**2 + (y[0][i] - y[1][j])**2)) if t1 == t2 else 90)
    ...
    for i in range(len(adjacent)):
        for j in range(len(adjacent[i])):
            edge_ops[i].append([int(7.5 * len(edge_types[i][j][k])) for k in range(len(adjacent[i]))])
        ...
        for j, types1 in enumerate(row1):
            for k, row2 in enumerate(edge_types[1]):
                for l, types2 in enumerate(row2):
                    substitutions.append(0 if len(types1) == 0 and len(types2) == 0 else int(edge_substitution_cost(types1, types2)))
